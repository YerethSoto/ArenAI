...
import { learningStatsService, SubjectStats } from "../services/learningStatsService";
...
const StudentProfile: React.FC = () => {
  ...
  // Find best subject
  const bestSubject = subjectStats.reduce((prev, current) => (prev.mastery > current.mastery) ? prev : current, {
    subject: "N/A", mastery: 0, averageScore: 0, quizzesTaken: 0, totalCorrect: 0, totalQuestionsAnswered: 0
  });
        ...
        <PageTransition>
          ...
          <div className="profile-hero-card">
            ...
            <div className="avatar-section-centered">
              ...
              <div className="avatar-frame-premium large" onClick={() => router.push("/character-detail")}>
                ...
                />
                <div className="edit-icon-badge" onClick={(e) => {
                  e.stopPropagation();
            ...
            <div className="player-identity-centered">
              ...
              <div className="xp-progress-container">
                <div className="xp-progress-bar">
                  <div className="xp-progress-fill" style={{ width: `${Math.min(100, xpPercentage * 100)}%` }}></div>
                </div>
                <div className="xp-text-detail">{xp} / {nextLevelXp} XP</div>
              </div>
            </div>

            {/* QUICK STATS BAR - Centered & Simplified */}
            <div className="quick-stats-scroll" style={{ justifyContent: 'center' }}>
          ...
          <div className="profile-content-container">
            ...
            {bestSubject.quizzesTaken > 0 && (
              <div className="stats-highlight-card stat-card-gold">
                ...
                <div className="highlight-info">
                  ...
                  <span className="highlight-sub">{bestSubject.averageScore}% {t("studentProfile.stats.average")}</span>
                </div>
              </div>
            )}
            ...
            <IonAccordionGroup className="parent-category-accordion">
              <IonAccordion value="subjects-category" className="parent-accordion-card">
                <IonItem slot="header" lines="none" className="category-header">
                  <div className="category-header-content">
                    <span className="category-title">{t("studentProfile.stats.learningStats")}</span>
                    <span className="category-badge">{subjectStats.length}</span>
                  </div>
                </IonItem>
                ...
                <div slot="content" className="category-content">
                  ...
                  <IonAccordionGroup className="subject-accordion-group">
                    {subjectStats.map((stat, index) => (
                      <IonAccordion key={index} value={stat.subject} className="subject-accordion">
                        <IonItem slot="header" color="light" lines="none" className="accordion-header-item">
                          <IonLabel>
                            <div className="ms-top-header">
                              <span className="ms-subject-title">{stat.subject}</span>
                              <span className="ms-score-badge">{stat.mastery}%</span>
                            </div>
                            <div className="mini-progress-bar">
                              <div style={{ width: `${stat.mastery}%`, height: '100%', background: 'var(--ion-color-primary)', borderRadius: '2px' }}></div>
                            </div>
                          </IonLabel>
                        </IonItem>
                        <div className="ion-padding accordion-content" slot="content">
                          <div className="stat-detail-row">
                            <div className="sd-item">
                              ...
                              <span className="sd-value">{stat.quizzesTaken}</span>
                            </div>
                            <div className="sd-item">
                              <span className="sd-label">Avg. Score</span>
                              <span className="sd-value">{stat.averageScore}%</span>
                            </div>
                            <div className="sd-item">
                              <span className="sd-label">Total Correct</span>
                              <span className="sd-value">{stat.totalCorrect}</span>
                            </div>
                          </div>
                        </div>
                      </IonAccordion>
