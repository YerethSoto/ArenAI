...
// Run with: node scripts/parseTopicsCSV.js

const fs = require('fs');
const path = require('path');

const csvPath = path.join(__dirname, '..', 'resources', 'temas al 3er ciclo.xlsx - Materias.csv');
const outputPath = path.join(__dirname, '..', 'src', 'data', 'topicsData.ts');
...
for (const line of dataLines) {
    ...
    if (matches) {
        ...
        const topicSeqId = parseInt(matches[3], 10);
...
// Generated by: node scripts/parseTopicsCSV.js
...
export const ALL_TOPICS: Topic[] = ${JSON.stringify(topics, null, 4).replace(/"([^"]+)":/g, '$1:')};
