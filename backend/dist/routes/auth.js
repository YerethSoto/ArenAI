...
router.post('/register', async (req, res, next) => {
    try {
        ...
        if (body.institution && body.institution.trim()) {
            const instResult = await db.query(`SELECT id_institution FROM institution WHERE name_institution = ? LIMIT 1`, [body.institution.trim()]);
...
router.post('/register-student', async (req, res, next) => {
    ...
    try {
        ...
        const secRes = await db.query(`SELECT id_section, id_institution FROM section WHERE id_section = ? LIMIT 1`, [body.sectionId]);
