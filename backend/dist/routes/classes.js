...
router.post('/:classId/topics', async (req, res, next) => {
    ...
    try {
        ...
        await assignTopicsToClass(classId, body.topics.map((topic) => ({
...
router.post('/:classId/students', async (req, res, next) => {
    ...
    try {
        const { classId } = paramsSchema.parse(req.params);
        const body = bodySchema.parse(req.body);
        await enrollStudentsInClass(classId, body.students.map((student) => ({
...
router.post('/:classId/topics/scores', async (req, res, next) => {
    ...
    try {
        const { classId } = paramsSchema.parse(req.params);
        const body = bodySchema.parse(req.body);
        await recordClassStudentTopicScores(classId, body.entries.map((entry) => ({
            userId: entry.userId,
